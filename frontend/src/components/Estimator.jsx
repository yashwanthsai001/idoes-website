...
export default function Estimator() {
  ...
  const reset = () => { setAnswers({}); setStepIdx(0); };

  return (
    <div data-testid="estimator" className="bg-[#0A0A0A] border border-black/30 p-8 lg:p-12">
      ...
      <h3 className="font-display text-white mt-3" style={{ fontSize: "clamp(32px, 4vw, 56px)" }}>
        GET A NUMBER. IN 3 CLICKS.
      </h3>

      {!range && (
        <div className="mt-10">
          <p className="text-white/70 text-base">{`STEP ${stepIdx + 1} / 3 — ${steps[stepIdx].title}`}</p>
      ...
      {range && (
        <div className="mt-10" data-testid="estimator-result">
          ...
          <p className="text-white/70 text-sm lg:text-base mt-3 max-w-xl">Ballpark for {answers.need.label.toLowerCase()}, {answers.timeline.label.toLowerCase()} timeline. Final scope confirmed in a 20-minute call.</p>
